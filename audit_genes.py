...
def audit_genes():
    ...
    try:
        ...
        for k in base_checks:
            if k not in existing_keys:
                print(f"  ❌ FAILED: Base feature '{k}' was not generated by precompute_base_features!")
                return False
        ...
        for k in derived_checks:
            if k not in existing_keys:
                print(f"  ❌ FAILED: Derived feature '{k}' was not generated! (Dependency parsing or JIT worker failed)")
                return False

    except Exception as e:
        print(f"  ❌ CRASH: Feature computation crashed: {e}")
        import traceback
        traceback.print_exc()
        return False
    finally:
    ...
    print("  ✅ Pipeline computed all required keys.")
    ...
    for gene in genes:
        try:
            ...
            if not isinstance(res, np.ndarray):
                print(f"  ❌ FAILED: {gene.type} returned {type(res)}, expected np.ndarray")
                return False
            if len(res) != n:
                 print(f"  ❌ FAILED: {gene.type} returned length {len(res)}, expected {n}")
                 return False
            ...
            print(f"  ❌ CRASH: {gene.type}.evaluate() failed: {e}")
            return False
            
    print("  ✅ All genes evaluated successfully.")
    print("\n🎉 AUDIT PASSED: Genome Integrity Verified.")
    return True
