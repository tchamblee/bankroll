...
def run_pipeline(engine, data_cache=None):
    ...
    Args:
        ...
        data_cache (dict): Dictionary containing external DataFrames:
                           tnx, us2y, zn, vix, ibit, gdelt, tick_nyse, trin_nyse.
    """
    ...
    # 3. Add Correlator Residuals (TNX for rates)
    ...
    # 8. Macro Voltage (US curve only for ES)
    # Refactor: Increased windows to [100, 200, 400] for stability
    engine.add_macro_voltage_features(
        us2y_df=get_df('us2y'),
        schatz_df=None,  # Removed EU rates
        tnx_df=get_df('tnx'),
        bund_df=None,  # Removed EU rates
        windows=[100, 200, 400]
    )
    ...
    # 12b. Economic Surprise Features
    if engine.bars is not None:
        engine.bars = add_econ_surprise_features(engine.bars)
...
def create_full_feature_engine(data_dir=None, volume_threshold=250):
    ...
    # 2. Load Shared Data (Correlators & Macro for ES)
