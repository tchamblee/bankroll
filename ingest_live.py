...
def download_v2_gkg_for_ts(ts_str: str) -> Optional[pl.DataFrame]:
    ...
    logger.info(f"[GKG2 {ts_str}] Fetch -> {url}")
    ...
    try:
        df = pl.read_csv(
            ...
            encoding="utf8-lossy" # Handle invalid utf-8 bytes
        )
    except Exception as e:
        logger.warning(f"[GKG2 {ts_str}] Failed to parse CSV: {e}")
        return None

    if df.width < 27:
        logger.warning(f"[GKG2 {ts_str}] Expected >=27 cols, got {df.width}.")
        return df
...
def ensure_v2_gkg_until(cutoff: dt.datetime):
    ...
    for ts in ts_list:
        ...
        if df is None or df.is_empty():
            logger.info(f"[GKG2 {ts_str}] No data ingested.")
            continue

        out_path = os.path.join(V2_GKG_DIR, f"gdelt_v2_gkg_{ts_str}.parquet")
        df.write_parquet(out_path)
        logger.info(f"[GKG2 {ts_str}] Wrote {df.height} rows -> {out_path}")
